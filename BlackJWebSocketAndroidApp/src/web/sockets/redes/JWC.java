...
/**
 * Método para manejar los conexiones con el servidor jWebSocket.
...
public class JWC {
	...
	private static String mURL = c.url;
	...
	private static String mUser = c.user;
	private static String mPass = c.pass;
	...
	public static void saveSettings(Activity aActivity) {
		...
		try {
			lProps.put("url", mURL);
			lProps.put("user", mUser);
			lProps.put("pass",mPass);
	...
	public static Properties getlProps() {
		return lProps;
	}

	public static void setlProps(Properties lProps) {
		JWC.lProps = lProps;
	}
